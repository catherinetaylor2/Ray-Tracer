...
//initial inputs
	...
	std::cout<<"tree built"<<root->right_node->number_of_node_faces<< "\n";
	...
	//	std::vector<vector3> colours;
	  //  TriangleColour::anti_aliasing(ratio, u,  v, eye, root,  V, N, FV, FN, area, A, GREY, sun, myscene, &colours, L, i-1/2.0f, j+1/2.0f, 0);
		...
		//std::cout<<"d "<<d.get_x()<<" "<<d.get_y()<<" "<<d.get_z()<<" "<<eye.get_x()<<" "<<eye.get_y()<<" "<<eye.get_z()<<"\n";
			vector3 RGB = TriangleColour::intersection_colour(d, eye, root,  V, N, FV, FN, area, A, RED, sun, myscene, output);
