...
class TriangleColour{
    public:
       ...
       static vector3 intersection_colour(vector3 direction, vector3 camera_origin, std::vector<search_tree*> root_data,  std::vector<float*> mesh_data, std::vector<int*> mesh_data_i, const int* tri_colour, Light sun, scene myscene, unsigned char* data, int texture_width, int texture_height);
       static void anti_aliasing(std::vector<vector3> scene_pos, std::vector<search_tree*> root_data,  std::vector<float*> mesh_data, std::vector<int*> mesh_data_i,  const int* tri_colour, Light sun, scene myscene, std::vector<vector3> *colours, vector3 L, float* ijit, unsigned char *data, int* texture_data);
